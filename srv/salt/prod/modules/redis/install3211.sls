...
redis-installed:
  ...
  pkg.installed:
    - pkgs:
      ...
      - gcc-c++
  cmd.run:
    - name: cd /usr/local/src/redis-{{ REDIS_VERSION }} && make && make PREFIX=/usr/local/redis install && ln -s /usr/local/redis/bin/redis-cli /usr/bin/redis-cli
    - require:
      - pkg: redis-installed
...
redis-cluser-installed:
  pkg.installed:
    - pkgs:
      - ruby
      - rubygems
  file.managed:
    - name: /usr/local/src/redis-{{ REDIS_VERSION }}.gem
    - source: salt://modules/files/redis-{{ REDIS_VERSION }}.gem
    - user: root
    - group: root
    - mode: 644
  cmd.run:
    - name: gem install /usr/local/src/redis-{{ REDIS_VERSION }}.gem
    - unless: gem list |grep redis
    - require:
      - pkg: redis-cluser-installed
      - file: redis-cluser-installed
